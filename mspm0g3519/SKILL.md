...
**1. OLED_Set_Pos 列地址 Bug** — 模板驱动中 `(x & 0x0f) | 0x01` 如仍存在则**必须改为** `| 0x00`。
   否则列 0 无法寻址，所有内容向右偏移 1 像素。
   ```c
   // 修复前 (错误)
...
**2. 寻址模式** — SSD1306 页寻址模式。

**3. 文字布局规则 [关键]** — 8×16 字体(SIZE=16)每个字符占 **2 个 page**（16像素高）。
   - **行间隔必须 ≥2 pages**，否则上下行文字会互相覆盖。
   - 4 行文字的唯一安全布局：pages **0-1, 2-3, 4-5, 6-7**。
   - **不要在 page 7 放文字**：字符下半截(page 8)超出屏幕，显示不全。

**4. 水平边界 [关键]** — 每个字符 8px 宽。
   - **起始 x + 字符数 × 8 ≤ 128**，否则右侧字符超出屏幕。

> **违反第 3、4 条是导致"内容重叠"和"内容溢出屏幕"类 bug 的第一大原因。**
